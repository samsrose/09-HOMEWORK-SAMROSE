...
function getTweets() {
  //Twitter with some Stanley Kubrick console logs woven in
  console.log("Please wait while I find your tweets.\n");
  var twitter = new Twitter(keys.twitter); //parse those keys from the earlier (e.g. key.js/.env)
  twitter.get("statuses/home_timeline", function(error, tweets, response) {
    //create request via Request plugin, formatting for twitter api GET
    if (tweets.length < 1) {
      console.log(
        "Good news. You don't have any tweets. You should be proud of yourself.\n"
      );
      return;
    }
    if (tweets.length === 1) {
      console.log("Poor Dave. Only one squawking:\n");
    } else {
      console.log(
        "Here are your " + tweets.length + " most recent squawkings:\n"
      );
    }
...
// OVER HAL-9000!!!
function NineThousandHertZ(song) {
  song = song || "The Sign";
  console.log("Just what do you think you're doing, Dave?\n");
  var spotify = new Spotify(keys.spotify);
  spotify
    .search({ type: "track", query: "track:" + song, limit: 20 })
    .then(function(response) {
      var foundSong = false;
      for (var i = 0; i < response.tracks.items.length; i++) {
        if (
          response.tracks.items[i].name.toLowerCase() === song.toLowerCase()
        ) {
          console.log(
            "I am putting myself to the fullest possible use... Here's some information what you have requested:\n"
          );
          if (response.tracks.items[i].artists.length > 0) {
            var artists =
              response.tracks.items[i].artists.length > 1
                ? "  Artists: "
                : "  Artist: ";
      ...
      if (!foundSong) {
        console.log(
          "I'm Sorry, I couldn't find any songs called '" +
            song +
            "' on Spotify."
        );
      }
...
function CinemaInfo(movie) {
  movie = movie || "Zoolander";
  var queryUrl = "https://www.omdbapi.com/?apikey=trilogy&s=" + movie; // using search becuase returning movies directly can be buggy
    ...
    if (body && JSON.parse(body).Search && JSON.parse(body).Search.length > 0) {
      for (var i = 0; i < JSON.parse(body).Search.length; i++) {
        ...
        if (result.Title.toLowerCase() === movie.toLowerCase()) {
          var cont = false;
          var innerQueryURL =
            "https://www.omdbapi.com/?i=" + result.imdbID + "&apikey=trilogy";
          request(innerQueryURL, function(error, response, body) {
            // another request because the search result doesn't give enough information
            if (error) {
            ...
            if (
              ...
              console.log(
                "I think I found the movie you were looking for. Here's some information on it:\n"
              );
              ...
              for (var j = 0; j < body.Ratings.length; j++) {
                if (body.Ratings[j].Source === "Internet Movie Database") {
                  ...
                  console.log(
                    "  Rotten Tomatoes Score: " + body.Ratings[j].Value
                  );
                }
              }
              console.log(
                "  " +
                  (body.Country.indexOf(",") < 0
                    ? "Country: "
                    : "Countries: ") +
                  body.Country
              );
      ...
      request(innerQueryURL, function(error, response, body) {
        if (error) {
          console.log(
            "I'm sorry, but I seem to have run into an error.\n  " + error
          );
          return;
        }
        if (body && JSON.parse(body) && JSON.parse(body).Response === "True") {
          body = JSON.parse(body);
          console.log(
            "Hmm. Maybe we don't speak the same language? Couldn't find anything that matched what you requested.\n This was the closest match I could find:\n"
          );
          console.log("  Title: " + body.Title);
          console.log("  Year: " + body.Year);
          for (var j = 0; j < body.Ratings.length; j++) {
          ...
          console.log(
            "  " +
              (body.Country.indexOf(",") < 0 ? "Country: " : "Countries: ") +
              body.Country
          );
          console.log(
            "  " +
              (body.Language.indexOf(",") < 0 ? "Language: " : "Languages: ") +
              body.Language
          );
          console.log("  Actors: " + body.Actors);
          console.log("  Plot: " + body.Plot);
      ...
      console.log(
        "I'm sorry, I wasn't able to find any movies by what you specified  '" +
          movie +
          "'. Make sure to use the exact name of the movie, otherwise I might not find it!"
      );
    }
  });
}
...
function zipZapZing() {
  fs.readFile("random.txt", "utf8", function(error, data) {
    if (error) {
      return console.log("YOU BROKE IT!\n  " + error);
    }
